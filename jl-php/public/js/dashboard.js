...
const cargaUsuarios = async () => {
  try {
    const res = await fetch('http://localhost:8888/jl-php/backend/index.php/usuarios')
...
const eliminarUsuario = async id => {
  ...
  if (confirmation) {
    try {
      const response = await fetch(`http://localhost:8888/jl-php/backend/index.php/usuario/${id}`, {
        method: 'DELETE'
      })
...
crearForm.addEventListener('submit', async (event) => {
  ...
  try {
    const response = await fetch('http://localhost:8888/jl-php/backend/index.php/create', {
      method: 'POST',
      headers: {
...
const abrirEditar = (usuario) => {
  ...
  editarForm.onsubmit = async (event) => {
    ...
    try {
      const response = await fetch(`http://localhost:8888/jl-php/backend/index.php/usuario/${usuario.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
